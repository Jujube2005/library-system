...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'role_enum') THEN
    CREATE TYPE public.role_enum AS ENUM ('student','instructor','staff');
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'book_status_enum') THEN
    CREATE TYPE public.book_status_enum AS ENUM ('available','borrowed','reserved','unavailable');
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'loan_status_enum') THEN
    CREATE TYPE public.loan_status_enum AS ENUM ('active','returned','overdue');
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'fine_status_enum') THEN
    CREATE TYPE public.fine_status_enum AS ENUM ('unpaid','paid');
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'reservation_status_enum') THEN
    CREATE TYPE public.reservation_status_enum AS ENUM ('pending','ready','fulfilled','cancelled','expired');
  END IF;
END$$;
